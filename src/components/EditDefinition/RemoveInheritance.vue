...
export default {
        ...
        superClassToRemove(index, superClassName) {
            ...
            for (let i in this.$store.state.currentFile.file[this.$store.state.isSelected]["allOf"]) {
                if (this.$store.state.currentFile.file[this.$store.state.isSelected]["allOf"][i]["$ref"]) {
                    if (this.$store.state.currentFile.file[this.$store.state.isSelected]["allOf"][i]["$ref"].includes(this.selectedSuperClass)) {
                        this.selectedSuperClassRefFile = miscUtilities.getRefFileContext(this.$store.state.currentFile.file[this.$store.state.isSelected]["allOf"][i]["$ref"])
                    }
                }
            }      
    ...
    computed: {
        ...
        defnDetails() {
            ...
            // console.log(defnName)
            let defnFile = this.$store.state.loadedFiles[this.selectedSuperClassRefFile]["file"]
            ...
            if (defnFile[superClass]["allOf"]) {
                ...
                for (let i in defnFile[superClass]["allOf"]) {
                    if (defnFile[superClass]["allOf"][i]["$ref"]) {
                        ...
                        temp_superClassList.push(defnFile[superClass]["allOf"][i]["$ref"]
                            .slice(defnFile[superClass]["allOf"][i]["$ref"].lastIndexOf("/") + 1))
                    } else {
                                                                // console.log('8')
